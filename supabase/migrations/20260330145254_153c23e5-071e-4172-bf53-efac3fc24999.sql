...
SELECT 
  COALESCE(cr.representative_email, c.owner_email) AS owner_email,
  o.client_id,
  COALESCE(c.trade_name, o.client_name) AS client_name,
  max(o.issue_date) AS last_purchase_date,
  CURRENT_DATE - max(o.issue_date)::date AS days_since_last_purchase,
  sum(
    CASE
...
LEFT JOIN (
  SELECT DISTINCT ON (client_id) client_id, representative_email
  FROM client_representatives
  ORDER BY client_id, created_at DESC
) cr ON cr.client_id = o.client_id
GROUP BY COALESCE(cr.representative_email, c.owner_email), o.client_id, COALESCE(c.trade_name, o.client_name);